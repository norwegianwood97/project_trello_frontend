...
function BoardPage() {
  ...
  const fetchBoardMembers = async () => {
    try {
      const response = await axios.get(`/api/boards/${boardId}/userBoard`);
    ...
    try {
      ...
      const columnsWithCards = await Promise.all(
        columnsData.map(async (column) => {
          const res = await axios.get(`/api/columns/${column.columnId}/cards`, {
            headers: { 'Cache-Control': 'no-cache' },
          });
  ...
  const handleAddCard = async () => {
    try {
      ...
      await axios.post(`/api/columns/${selectedColumnId}/cards`, newCard);
  ...
  const handleAddColumn = async () => {
    const url = `/api/boards/${boardId}/columns`; // 로깅을 위한 변수 추가
