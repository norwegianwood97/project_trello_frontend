...
function BoardPage() {
  ...
  const fetchBoardMembers = async () => {
    try {
      const response = await axios.get(`http://api.nodejstrello.site/api/boards/${boardId}/userBoard`);
  ...
  const fetchBoardInfo = async () => {
    try {
      const response = await axios.get('http://api.nodejstrello.site/api/boards');
  ...
  const fetchColumnsAndCards = async () => {
    try {
      const response = await axios.get(`http://api.nodejstrello.site/api/boards/${boardId}/columns`, {
      ...
      const columnsWithCards = await Promise.all(
        columnsData.map(async (column) => {
          const res = await axios.get(`http://api.nodejstrello.site/api/columns/${column.columnId}/cards`, {
            headers: { 'Cache-Control': 'no-cache' },
          });
  ...
  const handleAddCard = async () => {
    try {
      ...
      await axios.post(`http://api.nodejstrello.site/api/columns/${selectedColumnId}/cards`, newCard);
  ...
  const handleAddColumn = async () => {
    const url = `http://api.nodejstrello.site/api/boards/${boardId}/columns`; // 로깅을 위한 변수 추가
  ...
  const handleModifyColumn = async () => {
    ...
    try {
      await axios.put(`http://api.nodejstrello.site/api/boards/12/columns/${editingColumnId}`, updatedColumnData);
  ...
  const handleDeleteColumn = async () => {
    try {
      await axios.delete(`http://api.nodejstrello.site/api/boards/12/columns/${editingColumnId}`);
      setIsModifyColumnModalOpen(false);
