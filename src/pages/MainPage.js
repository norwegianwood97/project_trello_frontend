...
function MainPage() {
  ...
  useEffect(() => {
    ...
    axios
      .get('http://api.nodejstrello.site/api/user/get', {
        headers: {
    ...
    axios
      .get('http://api.nodejstrello.site/api/boards', {
        headers: {
          'Cache-Control': 'no-cache',
        },
      })
    ...
    axios
      .get('http://api.nodejstrello.site/api/boards/userBoard', {
        headers: {
          'Cache-Control': 'no-cache',
        },
      })
  ...
  const handleLogout = () => {
    axios.delete('http://api.nodejstrello.site/api/logout').then(() => navigate('/login')); // Make sure to navigate to the correct path
  };

  const handleWithdrawal = () => {
    axios.delete('http://api.nodejstrello.site/api/user').then(() => navigate('/login')); // Make sure to navigate to the correct path
  };
  ...
  const handleEditSubmit = (updatedData) => {
    axios
      .put('http://api.nodejstrello.site/api/user', updatedData)
      .then((response) => {
  ...
  const handleAddBoard = (newBoardData) => {
    ...
    axios
      .post('http://api.nodejstrello.site/api/boards', newBoardData)
      .then((response) => {
  ...
  const handleJoinSubmit = (boardCode) => {
    ...
    axios
      .post('http://api.nodejstrello.site/api/boards/joinPage', formData)
      .then((response) => {
        // 요청 성공 시의 로직
