...
function ColumnPage() {
  ...
  const fetchColumnTitle = async (columnId) => {
    try {
      const response = await axios.get(`http://api.nodejstrello.site:3000/api/columns/${columnId}/`);
  ...
  //     const currentUserResponse = await axios.get('http://api.nodejstrello.site:3000/api/user/');
  //     const currentUserId = currentUserResponse.data.userId;

  //     let response;
  //     // 현재 사용자인 경우
  //     if (currentUserId === userId) {
  //       response = await axios.get('http://api.nodejstrello.site:3000/api/user/');
  //     } else {
  //       // 다른 사용자인 경우
  //       response = await axios.post('http://api.nodejstrello.site:3000/api/user/', { userId });
  //     }
  ...
  const fetchCards = async (columnId) => {
    try {
      const response = await axios.get(`http://api.nodejstrello.site:3000/api/columns/${columnId}/cards`);
  ...
  const handleDelete = async (columnId, cardId) => {
    if (window.confirm('Are you sure you want to delete this card?')) {
      try {
        await axios.delete(`http://api.nodejstrello.site:3000/api/columns/${columnId}/cards/${cardId}`);
        alert('카드가 삭제되었습니다!');
        fetchCards();
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      if (editMode) {
        response = await axios.put(`http://api.nodejstrello.site:3000/api/columns/14/cards/${editCardId}`, cardPayload);
        setEditMode(false);
        setEditCardId(null);
      } else {
        response = await axios.post('http://api.nodejstrello.site:3000/api/columns/14/cards', cardPayload);
      }
